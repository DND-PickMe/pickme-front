export const PATH_WELCOME = '/';
export const PATH_SIGN_IN = '/sign-in';
export const PATH_SIGN_UP = '/sign-up';
export const PATH_AUTH_CHECK = '/auth-check';

export const PATH_MAIN = '/main';
export const PATH_EXPLORE = '/explore';
export const PATH_ENTERPRISES = '/enterprises';

export const PATH_USER_PROFILE = '/user/my-profile';
export const PATH_ADD_RESUME = '/user/add-resume';

export const PATH_RESUME = '/resume/:id'
export const PATH_ENTERPRISE = '/enterprises/:id'
